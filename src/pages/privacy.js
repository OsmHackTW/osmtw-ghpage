            ...
            <section className="antialiased text-gray-900 py-2 h-full flex items-center justify-center">
                <div className="w-full xl:w-8/12 2mx-auto px-4 sm:px-8  pb-16">
                    <div className="main-title my-8 md:my-16">
                        ...
                        <h1 className="font-bold text-gray-500 text-lg md:text-3xl lg:text-4xl text-center my-4">Privacy Statement</h1>
                        <h2 className="font-bold text-grey-400 text-center my-4">最新更新於 2021-03-19</h2>
                    </div>
                    <div className="bg-white shadow overflow-hidden sm:rounded-lg">
                        <div className="px-4 py-5 sm:px-6">
                            <h3 className="flex flex-col text-lg md:text-xl leading-6 font-medium text-gray-900">
                        ...
                        <div className="border-t border-gray-200">
                            <dl> {privacyPageComponents.privacyArticle.map((article, index) => (
                                <div key={index} className={'bg-' + (index % 2 ? 'gray-100' : 'white') + ' px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6'}>
                                    ...
                                    <dd className="mt-1 text-sm text-fern-green-800 sm:mt-0 sm:col-span-2">
                                        {article.text}
                                        {article.point === "SectionV" ? <ul className="border border-gray-200 mt-4 rounded-md divide-y divide-gray-200">
                                            {privacyPageComponents.privacyAritcleSectionV.map((pSV) => (
                                                <li className="pl-3 pr-4 py-3 flex items-center justify-between text-sm">{pSV}</li>)
                                            )}</ul> : ""
                                        }
                                    </dd>
                                </div>))}
