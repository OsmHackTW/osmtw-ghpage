        ...
        <div className="bg-white flex flex-col flex-wrap p-6 max-w-7xl mx-auto overflow-hidden">
          ...
          <div className="my-8 space-y-6">
            {landingPageComponents.quickStartGuide.map((item) => (
              <Link to={item.route} className="flex flex-col lg:flex-row border rounded shadow-md my-3 p-4 justify-between">
                <h2 className="text-fern-green-500">{item.role}</h2>
                <h2 className="text-sm text-gray-400">{item.route.replace('/qsg', '')}</h2>
              </Link>
            ))}
          </div>
        </div>
